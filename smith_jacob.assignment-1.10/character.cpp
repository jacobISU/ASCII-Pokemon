...
// This is a table holding the move cost for each type of enemy.
// Since turns are based on heap, the lower cost to move, moves first.
int32_t move_cost[num_character_types][num_terrain_types] = {
    ...
    {IM, IM, 10, 50, 50, 20, 10, IM, IM, IM, IM}, // Trainer
};
#undef IM
...
// Is a specific cell (pos) on the game grid adjacent to another cell (ter).
...
uint32_t can_see(map_t *m, character *voyeur, character *exhibitionist)
{
  ...
  /* For the purposes of this project, can swimmers see the PC adjacent to water or on a bridge?  
...
// When the main character '@' moves, update the game board.
...
//Path finding algorithm for hikers and rivals. 
void pathfind(map_t *m)
{
  heap_t h;
  uint32_t x, y;
